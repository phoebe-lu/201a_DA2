...
longplot <- data.table(copy(cdi))
longplot <- melt(longplot,id.vars=c("id","cty","state","region","region_f"))
histdat <- copy(longplot[variable %in% c("pop18","exp_hsgrad", "log_bagrad","log_poverty","log_unemp","log_pcincome","log_crm_1000")])
histdat[,fact_var:=factor(variable,levels=c("pop18","exp_hsgrad", "log_bagrad","log_poverty","log_unemp","log_pcincome","log_crm_1000"),
...
sr.2 <- lm(log_crm_1000 ~ exp_hsgrad, data=cdi)
...
#pop18, log_poverty, region_f, log_bagrad, log_pcincome, exp_hsgrad, log_unemp

#Manual stepwise regression (backwards)
#Total model
mr.1 <- lm(log_crm_1000 ~ pop18 + log_poverty + region_f + log_bagrad + log_pcincome + exp_hsgrad + log_unemp, data=cdi)
summary(mr.1) #Adj r2: 0.4833
AIC(mr.1) #365.7378

#Total model-log_unemp is an improvement over Model 1
mr.2 <- lm(log_crm_1000 ~ pop18 + log_poverty + region_f + log_bagrad + log_pcincome + exp_hsgrad, data=cdi)
summary(mr.2) #Adj r2: 0.4841
AIC(mr.2) #364.0604

#Total model-log_ump, exp_hsgrad is an improvement over Model 2
mr.3 <- lm(log_crm_1000 ~ pop18 + log_poverty + region_f + log_bagrad + log_pcincome, data=cdi)
summary(mr.3) #Adj r2: 0.4853
AIC(mr.3) #362.0658

#Total model-log_ump, exp_hsgrad is not an improvement over Model 3
...
#Total model-log_ump, exp_hsgrad, log_bagrad is an improvement over Model 3; best model (all p-values < 0.05)
